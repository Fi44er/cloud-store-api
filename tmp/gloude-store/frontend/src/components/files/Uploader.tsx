...
export default function Uploader({ quota, onUploadComplete }: UploaderProps) {
  ...
  const processFiles = useCallback(async (files: FileList | File[]) => {
    ...
    if (quota) {
      ...
      if (totalUploadSize > quota.free) {
        toast.error(`Недостаточно места! Нужно ${formatFileSize(totalUploadSize)}, доступно ${formatFileSize(quota.free)}`);
        return;
      }
    }
    ...
    for (let i = 0; i < fileArray.length; i++) {
      ...
      try {
        await storageApi.upload(file, (progress) => {
          setUploads((prev) =>
            prev.map((u, idx) =>
      ...
      >
        ...
        {quota && (
          <p className="text-xs text-slate-600 mt-3">
            Доступно: {formatFileSize(quota.free)}
          </p>
        )}
      </div>
